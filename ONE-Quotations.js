...
// 适配浅色、深色模式内容字体颜色
const themes_Fontcolor = Color.dynamic(Color.black(), Color.black());
// 适配浅色、深色模式内容字体颜色
const themes_Maincolor = Color.dynamic(Color.black(), Color.black());
// 小尺寸设置边距(上，左，下，右)
...
async function createWidget() {
  ...
  switch (previewSize) {
    case "small":
      ...
      if (SFSymbolmodel) {
        let SdocsSymbol = SFSymbol.named("figure.wave");
    ...
    case "medium":
      let McenterStack = w.addStack();
      McenterStack.layoutVertically();
      McenterStack.centerAlignContent();

      let MaImage = await getImageByUrl(
        avatar,
        `_${Script.name()}-avatar`,
        false
      );
      let MprofileImage = McenterStack.addImage(MaImage);
      MprofileImage.imageSize = new Size(40, 40);
      MprofileImage.cornerRadius = 20;
      MprofileImage.borderWidth = 2;
      MprofileImage.borderColor = new Color("#bfbfbf");

      McenterStack.addSpacer(10);

      let Musername = McenterStack.addText(QuotationsData.title);
      Musername.font = Font.systemFont(14);
      Musername.textColor = themes_Fontcolor;
      Musername.centerAlignText();

      McenterStack.addSpacer(5);

      let Mdesc = McenterStack.addText(QuotationsData.desc);
      Mdesc.font = Font.systemFont(12);
      Mdesc.textColor = new Color("#bfbfbf");
      Mdesc.centerAlignText();

      w.addSpacer();

      let Mintro = w.addText("这里是介绍文本");
      Mintro.font = Font.systemFont(10);
      Mintro.textColor = new Color("#888888");
      Mintro.centerAlignText();
    ...
    case "large":
      ...
      Ltaglist.lineLimit = 1;

      LfooterStack.addSpacer();

      if (SFSymbolmodel) {
        let LdocsSymbol = SFSymbol.named("figure.wave");
